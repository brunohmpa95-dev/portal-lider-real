...
const SearchBar = () => {
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-3.5 sm:p-5 shadow-lg w-full max-w-3xl mx-auto">
      {/* Purpose toggle */}
      <div className="flex mb-3 bg-secondary rounded-lg p-0.5">
        <button
      ...
      {(!isMobile || showFilters) && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 mt-2.5">
          <Input className="h-10" placeholder="Valor mínimo" type="number" value={priceMin} onChange={e => setPriceMin(e.target.value)} aria-label="Valor mínimo" />
          <Input className="h-10" placeholder="Valor máximo" type="number" value={priceMax} onChange={e => setPriceMax(e.target.value)} aria-label="Valor máximo" />
          <Input className="h-10" placeholder="Código (ex: LDR001)" value={code} onChange={e => setCode(e.target.value)} aria-label="Código do imóvel" />
        </div>
      )}

      {/* Search button */}
      <Button onClick={handleSearch} className="w-full h-10 mt-3 gap-2 text-sm font-semibold">
