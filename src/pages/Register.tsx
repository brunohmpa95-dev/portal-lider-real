      ...
      <Layout>
        ...
        <div className="container mx-auto px-4 py-12 sm:py-16">
          <div className="max-w-sm mx-auto text-center bg-card border border-border rounded-lg p-6">
            <h1 className="text-lg font-sans font-bold text-foreground mb-2">Verifique seu e-mail</h1>
            <p className="text-xs text-muted-foreground mb-5">
              Enviamos um link de confirmação para <strong>{email}</strong>. Clique no link para ativar sua conta.
            </p>
            <Link to="/login">
              <Button variant="outline" size="sm">Ir para o login</Button>
            </Link>
          </div>
        </div>
      </Layout>
      ...
      <div className="container mx-auto px-4 py-12 sm:py-16">
        <div className="max-w-sm mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 bg-card border border-border rounded-lg p-5">
            ...
            <div>
              <label htmlFor="fullName" className="block text-xs font-medium text-foreground mb-1">Nome completo</label>
              <Input id="fullName" placeholder="Seu nome" value={fullName} onChange={e => setFullName(e.target.value)} required maxLength={100} autoComplete="name" disabled={isSubmitting} className="h-10" />
            </div>

            <div>
              <label htmlFor="email" className="block text-xs font-medium text-foreground mb-1">E-mail</label>
              <Input id="email" type="email" placeholder="seu@email.com" value={email} onChange={e => setEmail(e.target.value)} required maxLength={255} autoComplete="email" disabled={isSubmitting} className="h-10" />
            </div>

            <div>
              <label htmlFor="password" className="block text-xs font-medium text-foreground mb-1">Senha</label>
            ...
            <div className="flex items-start gap-2">
              ...
              <label htmlFor="terms" className="text-[11px] text-muted-foreground leading-tight">
                Li e concordo com a <Link to="/privacidade" className="text-primary hover:underline">Política de Privacidade</Link>.
              </label>
            </div>
